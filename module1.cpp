...
public:
    ...
    void addVehicle() 
    {
        if (vehicleCount < MAX_VEHICLES) 
        {
            ...
            cout << "Enter start intersection (e.g., A): ";
            cin >> startIntersection;

            cout << "Enter end intersection (e.g., B): ";
            cin >> endIntersection;
...
private:
    ...
    // Adjacency matrix
    int adjMatrix[MAX_INTERSECTIONS][MAX_INTERSECTIONS]; 
    // Heuristic values for A*
    int heuristic[MAX_INTERSECTIONS];                   
    ...
    void aStar(int start, int end, int parent[]) {
        ...
        // Cost from start to each intersection
        int gScore[MAX_INTERSECTIONS]; 
        // Estimated cost (gScore + heuristic)
        int fScore[MAX_INTERSECTIONS]; 
...
public:
    ...
    // Read road network data from CSV
    void readRoadNetwork() {
        ...
        while (getline(file, line)) {
            ...
            adjMatrix[idx2][idx1] = time; 
        }
    }

    // Read traffic signals from CSV
    void readTrafficSignals() {
        ...
        while (getline(file, line)) {
            ...
            // Use greenTime as heuristic for simplicity
            heuristic[idx] = greenTimeVal; 
        }
    }

    // Read road closures from CSV
    ...
    // Route emergency vehicle using A* algorithm
